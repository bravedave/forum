...
7. **Permissions**: Should items inherit project permissions, or have individual access control? Use `access_control()` method in controllers?

8. **Comments/discussions**: Add `forum_item_comments` table for threaded discussions on items?

9. **Notifications**: Email/push notifications when assigned, mentioned, or item status changes?
